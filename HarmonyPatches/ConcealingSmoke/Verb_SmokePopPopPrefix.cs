    ...
    public class Verb_SmokePopPopPrefix
    {
        public static bool Prefix(CompReloadable comp)
        {
            if (comp != null && comp.CanBeUsed)
            {
                ...
                wearer.Map.GetComponent<GasMapComponent>().gasGrids.Find(x => x.def == DefOfClass.GasExpansion_ConcealingSmoke).CreateGas(wearer.Position, parent.GetStatValue(StatDefOf.SmokepopBeltRadius) * 1250f);
